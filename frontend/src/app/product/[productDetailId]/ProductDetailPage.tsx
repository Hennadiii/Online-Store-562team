// ЭТОТ ФАЙЛ: src/app/product/[productDetailId]/ProductDetailPage.tsx
// Клієнтський компонент з усім UI. 
// Це колишній вміст page.tsx — вёрстка не змінилась, 
// лише тип пропса і імпорти оновлені.
  ...
  return (
    <section className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      {recentlyViewed.length > 0 && (
        <>
          <h3 className="mt-20 text-2xl font-semibold uppercase">Переглянуті</h3>
            ...
            {recentlyViewed.map((item) => (
              <Link href={`/product/${item.id}`} key={item.id} className="min-w-[160px] flex-shrink-0">
                <div className="aspect-[3/4] bg-gray-100 flex items-center justify-center group cursor-pointer overflow-hidden rounded-lg">
                  <Image src={item.images[0]} alt={item.title} width={180} height={240}
          ...
          <div className="hidden lg:grid mt-8 mb-20 grid-cols-4 gap-6">
            {recentlyViewed.map((item) => (
              <Link href={`/product/${item.id}`} key={item.id}>
                <div className="aspect-[3/4] bg-gray-100 flex items-center justify-center group cursor-pointer overflow-hidden rounded-lg">
                  <Image src={item.images[0]} alt={item.title} width={250} height={170}
                    className="transition-transform duration-300 group-hover:scale-105" />
                </div>

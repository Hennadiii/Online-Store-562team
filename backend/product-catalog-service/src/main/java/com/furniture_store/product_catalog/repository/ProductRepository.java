...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select p from Product p join fetch p.images where cast(:filters as boolean) order by :sort")
    Page<Product> findAllWithFilters(String filters, String sort, Pageable page);

    @Query(nativeQuery = true, value = "select * from product p join image where cast(lower(p.name) like concat('%', lower(:key), '%')" +
